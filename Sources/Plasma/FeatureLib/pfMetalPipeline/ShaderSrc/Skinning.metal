...
kernel void SkinningFunction(
    ...
    for (int j = 0; j < numWeights + 1; ++j) {
        ...
        if (weights[j]) {
            const float4x4 matrix = transpose(matrixPalette[index].matrix);
            destPoints.xyz += weights[j] * (matrix * float4(in.position, 1.0f)).xyz;
            destNorm.xyz += weights[j] * (matrix * float4(in.normal, 0.0f)).xyz;
        }
        indices >>= 8;
    }
